# @claude-context-cache/context-store

Tiered context store for [Claude Context Cache](../README.md). One small Node.js package, one `ContextStore` interface, six pluggable backends.
...
npm install redis            # for the Redis cache tier
npm install mysql2           # for MySQL backing
npm install pg               # for Postgres backing
npm install mongodb          # for MongoDB backing
```
